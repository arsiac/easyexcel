...
public class GenerateContextImpl implements GenerateContext {
    ...
    public GenerateContextImpl(OutputStream out, ExcelType excelType, boolean needHead) {
    ...
    private void buildHead(List<List<String>> head, Class<? extends ExcelRowModel> clazz) {
        if (head != null || clazz != null) { excelHeadProperty = new ExcelHeadProperty(clazz, head); }
    }

    public void appendHeadToExcel() {
        if (this.excelHeadProperty.getHead() != null && !this.excelHeadProperty.getHead().isEmpty()) {
